...
def main(config_path: str):
    ...
    print(f"[cascade:{split_name}] scoring PatchCore for unknown test...")
    ...
    print(f"[cascade:{split_name}] computed classifier confidences")
    ...
    # 2) system-level metrics on all samples
    ...
    # System-level rates include stage-1 leakage.
    ...
    metrics = {
        ...
        # Backward-compatible keys kept as conditional metrics.
        "auroc_known_unknown_conf": float(auroc_conf),
        "tpr_unknown": tpr_unknown_conditional,
        "fpr_known": fpr_known_conditional,
        "tpr_unknown_system": tpr_unknown_system,
        "fpr_known_system": fpr_known_system,
        "stage1_pass_rate_known": stage1_pass_rate_known,
        "stage1_pass_rate_unknown": stage1_pass_rate_unknown,
        "stage1_leakage_rate_known": float(np.mean(known_no_defect)),
        "stage1_leakage_rate_unknown": float(np.mean(unk_no_defect)),
        "stage1_pass_count_known": int(np.sum(known_mask)),
        "stage1_pass_count_unknown": int(np.sum(unk_mask)),
        "stage1_total_known": int(len(known_mask)),
        "stage1_total_unknown": int(len(unk_mask)),
    ...
    print(f"[cascade:{split_name}] done in {time.time() - t0:.1f}s")
