...
def main(config_path: str):
    ...
    print(f"[osr:{split_name}] loaded embeddings")
    print(f"  known_train: {len(known_train['labels'])}")
    print(f"  known_val: {len(known_val['labels'])}")
    print(f"  known_test: {len(known_test['labels'])}")
    print(f"  unknown_test: {len(unknown_test['labels'])}")
    ...
    print(f"[osr:{split_name}] fitting class Gaussians (cov_reg_lambda={cov_reg_lambda})")
    ...
    print(f"[osr:{split_name}] calibrated tau={tau:.6f} (accept_rate={cfg['osr']['accept_rate']})")
    ...
    if cfg["osr"]["use_confidence_gate"]:
        ...
        kappa = -kappa
        print(f"[osr:{split_name}] calibrated kappa={kappa:.6f} (accept_rate={cfg['osr']['confidence_accept_rate']})")
    else:
        kappa = None
        print(f"[osr:{split_name}] confidence gate disabled")
    ...
    print(f"[osr:{split_name}] done in {time.time() - t0:.1f}s")
